    ...
    <div class="board-viewport relative" :style="boardTransform">
      ...
      >
        ...
        <div
          ...
          :style="pieceStyle(p)"
        >
          <BoardPiece :type="p.type" :owner="p.owner" :amplitude="p.amplitude" />
        </div>
      </div>
...
import WaveOverlay from "@/components/WaveOverlay.vue";
...
function tileStyle(x: number, y: number) {
  ...
  return {
    background: (x + y) % 2 === 0 ? "linear-gradient(180deg, rgba(255,255,255,0.02), rgba(0,0,0,0.06))" : "transparent",
